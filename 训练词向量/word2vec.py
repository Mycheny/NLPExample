...
class word2vec():
    ...
    def __init__(self,
        ...
        if model_path != None:  # 如果模型已经训练好，可以直接加载模型
            ...
            self.id2word = {}  # word => id 的映射
            for i in range(self.vocab_size):
                self.word2id[self.vocab_list[i]] = i
                self.id2word[i] = self.vocab_list[i]
...
def get_stop_words():
    stop_words = []
    with open('stopword', encoding='utf-8') as f:  # 读取stop_words.txt，每行为一个词
...
def get_datasets():
    all_word_list_filename = "./temp/all_word_list.pkl"
    sentence_list_filename = "./temp/sentence_list.pkl"
    all_word_list = []  # 用于记录所有的词，即词典
    sentence_list = []  # 用于记录所有样本的样本list,如：[["今天","天气","不错"],["我","是","谁"],...,["","",""]]
    line_sum = 1
    if not os.path.exists(all_word_list_filename) and not os.path.exists(sentence_list_filename):
        with open('斗破苍穹.txt', encoding='gbk') as f:
            line = f.readline()  # 按行读取
            while line:  # 判断有没有读取到文件末尾，没有就执行循环语句
                if line_sum % 5000 == 0: print("正在处理第", line_sum, "行")
                while '\n' in line:
                    line = line.replace('\n', '')  # 将'\n'替换为''
                while ' ' in line:
        ...
        with open(all_word_list_filename, 'wb') as f:
            pickle.dump(all_word_list, f)
        with open(sentence_list_filename, 'wb') as f:
            pickle.dump(sentence_list, f)
    else:
        with open(all_word_list_filename, 'rb') as f:
            all_word_list = pickle.load(f)
        with open(sentence_list_filename, 'rb') as f:
            sentence_list = pickle.load(f)
    return all_word_list, sentence_list
...
if __name__ == '__main__':
    ...
    all_word_list, sentence_list = get_datasets()
    ...
    w2v = word2vec(vocab_list=word_list,  # 词典集
                   ...
                   logdir=model_path)  # tensorboard记录地址
    ...
    with codecs.open(model_path + '/metadata.tsv', 'w', encoding="utf-8") as f:
        f.write("Index\tLabel\n")
        for index, word in enumerate(tqdm(w2v.id2word)):
            f.write("%d\t%s\n" % (index, w2v.id2word[word]))
    config = projector.ProjectorConfig()
    embedding_conf = config.embeddings.add()
    embedding_conf.tensor_name = 'embedding:0'
    embedding_conf.metadata_path = os.path.join('metadata.tsv')
    projector.visualize_embeddings(w2v.summary_writer, config)
