...
p, h, y = load_char_data('./data/ATEC/processed/train.tsv', data_size=None)
p_eval, h_eval, y_eval = load_char_data('./data/ATEC/processed/dev.tsv', data_size=args.batch_size)
...
with tf.Session(config=config)as sess:
    ...
    model_path  = tf.train.latest_checkpoint(f'./model_ATEC/')
    print(model_path)
    ...
    for epoch in range(args.epochs):
        for step in range(steps):
            ...
            if step%10==0:
                print('epoch:', epoch, ' step:', f"{step}/{steps}", ' loss: ', loss, ' acc:', acc, 'cm', cm)
        ...
        print('\n')
        saver.save(sess, f'./model_ATEC/esim_{epoch}.ckpt')
