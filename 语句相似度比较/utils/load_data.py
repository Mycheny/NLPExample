...
import pandas as pd
import os
from 语句相似度比较.utils.data_utils import shuffle, pad_sequences
# from 语句相似度比较.dssm import args
import jieba
import re
from gensim.models import Word2Vec
import numpy as np
from 语句相似度比较.drcn import args


# 加载字典
def load_char_vocab():
    # path = os.path.join(os.path.dirname(__file__), '../input/vocab.txt')
    path = os.path.join(os.path.dirname(__file__), '../data/LCQMC/processed/vocab.txt')
...
# 加载char_index训练数据
def load_char_data(file, data_start=0, data_size=None):
    path = os.path.join(os.path.dirname(__file__), '../' + file)
    # df = pd.read_csv(path, sep='\t', error_bad_lines=False, quoting=csv.QUOTE_NONE)
    df = pd.read_csv(path, sep='\t')
    p = df['sentence1'].values[data_start:data_size]
    h = df['sentence2'].values[data_start:data_size]
    label = df['label'].values[data_start:data_size]
